...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, onSave, currentConfig }) => {
  ...
  const handleSave = () => {
    ...
    if (secretId.trim() || secretKey.trim()) {
      if (!secretId.trim() || !secretKey.trim()) {
        alert("如果使用腾讯云，请输入完整的 SecretId 和 SecretKey");
        return;
      }
      onSave({ secretId, secretKey, region });
    } else {
      // Allow saving if only Gemini key is provided
      onSave({ secretId: '', secretKey: '', region });
    }

    onClose();
    // 强制刷新页面以应用新的 API 密钥（简单粗暴但有效）
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden max-h-[90vh] overflow-y-auto">
        <div className="bg-slate-900 px-6 py-4 flex justify-between items-center border-b border-slate-700">
          <h3 className="text-white font-semibold text-lg">API 设置</h3>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            <h4 className="font-medium text-slate-900 border-b pb-2">Gemini 配置 (推荐)</h4>
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                没有密钥？<a href="https://aistudio.google.com/apikey" target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">点击获取</a>
              </p>
            </div>
          </div>
          ...
          {/* Tencent Settings */}
          <div className="space-y-4">
            <h4 className="font-medium text-slate-900 border-b pb-2">腾讯云配置 (可选)</h4>
            <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 flex items-start space-x-3 text-emerald-800 text-xs">
              ...
              <p>
                <strong>CORS 代理已启用：</strong> 请求将通过 <code>corsproxy.io</code> 安全路由。<br />
